...
class TestLenticrypt(unittest.TestCase):
    def setUp(self):
        self.plaintexts = tuple(random_string(random.randint(1,1024)) for _ in range(3))
        while True:
            self.keys = tuple(random_string(random.randint(2**14,2**15)) for _ in range(3))
            self.substitution_alphabet = lenticrypt.find_common_nibble_grams(self.keys)
            if len(self.substitution_alphabet[1]) >= 16**3:
                break

    def test_basic_encryption(self):
        ciphertexts = itertools.tee(lenticrypt.Encrypter(self.substitution_alphabet, [BytesIO(s) for s in self.plaintexts]), len(self.plaintexts))
        first_length = None
    ...
    def test_checksum_encryption(self):
        ciphertexts = itertools.tee(lenticrypt.LengthChecksumEncrypter(self.substitution_alphabet, [BytesIO(p) for p in self.plaintexts]), len(self.plaintexts))
        for key, plaintext, ciphertext in zip(self.keys, self.plaintexts, ciphertexts):
            decrypted = bytes(lenticrypt.decrypt(ciphertext, BytesIO(key)))
            self.assertEqual(decrypted, plaintext)

    def test_dictionary_encryption(self):
        ciphertexts = itertools.tee(lenticrypt.DictionaryEncrypter(self.substitution_alphabet, [BytesIO(p) for p in self.plaintexts]), len(self.plaintexts))
        for key, plaintext, ciphertext in zip(self.keys, self.plaintexts, ciphertexts):
            decrypted = bytes(lenticrypt.decrypt(ciphertext, BytesIO(key)))
            self.assertEqual(decrypted, plaintext)
